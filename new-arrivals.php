...
$products = $pdo->query("SELECT p.*, c.name as category_name FROM products p LEFT JOIN categories c ON p.category_id = c.id WHERE p.is_new = 1 ORDER BY p.id DESC")->fetchAll();
?>
...
<section class="py-5 bg-dark border-bottom border-secondary text-center">
    <div class="container" data-aos="fade-up">
        ...
        <h1 class="font-serif text-white gold-gradient-text display-4">New Arrivals</h1>
        <p class="text-muted small">The latest haute couture releases and newly faceted solitaire jewels.</p>
    </div>
</section>
