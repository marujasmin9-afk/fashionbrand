...
$products = $pdo->query("SELECT p.*, c.name as category_name FROM products p LEFT JOIN categories c ON p.category_id = c.id WHERE p.is_flash_sale = 1 OR (p.discount_price IS NOT NULL AND p.discount_price < p.price)")->fetchAll();
?>

<section class="py-5 bg-dark border-bottom border-secondary text-center">
    <div class="container" data-aos="fade-up">
        <span class="sub-title text-warning"><i class="fas fa-bolt me-1"></i> Exclusive Reductions</span>
        <h1 class="font-serif text-white gold-gradient-text display-4">Private Salon Sale</h1>
        <p class="text-muted small">Limited-time privilege pricing on selected haute couture and fine gemology.</p>
    </div>
</section>
